            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="flex border-b border-slate-100 bg-slate-50">
                    <button @click="tab = 'reglas'"
                        :class="tab === 'reglas' ? 'bg-white text-[#c5a059] border-t-4 border-t-[#c5a059]' :
                    ...
                    <button @click="tab = 'puntos'"
                        :class="tab === 'puntos' ? 'bg-white text-[#c5a059] border-t-4 border-t-[#c5a059]' :
                            'text-slate-400 hover:text-slate-600'"
                        class="flex-1 py-5 px-6 font-bold uppercase text-[10px] tracking-[0.2em] transition-all outline-none flex items-center justify-center gap-2">
                ...
                <div class="p-8 md:p-10">
                    ...
                    <div x-show="tab === 'reglas'" x-transition.opacity>
                        <div class="flex items-center gap-3 mb-6">
                            <h3 class="text-2xl font-black text-slate-800 uppercase italic tracking-tight">Manual de
                                Juego</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-slate-600">
                            {{-- Formato --}}
                            <div
                                class="flex items-start gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-[#c5a059]/30 transition-colors">
                                <div
                                    class="bg-[#c5a059] text-white w-8 h-8 rounded-lg flex items-center justify-center font-black flex-shrink-0 shadow-md">
                                    01</div>
                                <div>
                                    <p class="font-black text-slate-800 uppercase text-[11px] mb-1 tracking-widest">
                                        Formato de Encuentro</p>
                                    <p>Cada equipo compite en <strong>10 rondas</strong>. Los duelos se dividen en
                                        tableros específicos (A vs A, A vs B, C vs D), asegurando que cada miembro
                                        juegue 10 partidas individuales.</p>
                                </div>
                            </div>
                            ...
                            <div
                                class="flex items-start gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-[#c5a059]/30 transition-colors">
                                <div
                                    class="bg-slate-900 text-white w-8 h-8 rounded-lg flex items-center justify-center font-black flex-shrink-0 shadow-md">
                                    02</div>
                                <div>
                                    <p class="font-black text-[#c5a059] uppercase text-[11px] mb-1 tracking-widest">
                                        Sistema de Desempate</p>
                                    <p>Si hay empate en <strong>Puntos Globales</strong> al final del torneo, el ganador
                                        se decidirá por los <strong>Puntos de Mesa</strong> acumulados (puntos de todas
                                        las partidas individuales).</p>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <div>
                                    <p class="font-black text-[#c5a059] uppercase text-[11px] mb-1 tracking-widest">
                                        Ritmo y Reloj</p>
                                    <p>Es obligatorio el uso de <strong>Chessclock</strong>. El ritmo es <strong>Blitz
                                            (5+0)</strong>. No se permite ayuda externa (Fair Play estricto).</p>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <div>
                                    <p class="font-black text-slate-800 uppercase text-[11px] mb-1 tracking-widest">
                                        Leyes de la FIDE</p>
                                    <p>Se aplica la regla de <strong>Pieza Tocada, Pieza Movida</strong>. Una vez
                                        completado el movimiento legal, no hay marcha atrás.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-slate-100 pt-10">
                            <h4
                                class="text-center text-xl font-black text-slate-800 uppercase tracking-tighter mb-8 italic">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                                {{-- ORO --}}
                                <div
                                    class="bg-gradient-to-b from-yellow-50 to-white p-6 rounded-3xl border border-yellow-200 text-center shadow-sm hover:shadow-md transition-shadow">
                                    <div class="flex justify-center -space-x-3 mb-4">
                                        @for ($i = 0; $i < 4; $i++)
                                            <span class="text-4xl drop-shadow-md animate-bounce"
                                                style="animation-delay: {{ $i * 0.1 }}s">🥇</span>
                                        @endfor
                                    </div>
                                    <p class="font-black text-yellow-700 uppercase text-xs tracking-widest">1er Puesto
                                    </p>
                                    <p class="text-[10px] text-yellow-600/70 font-bold mt-1">4 Medallas de Oro<br>(Una
                                        para cada integrante)</p>
                                </div>

                                {{-- PLATA --}}
                                <div
                                    class="bg-gradient-to-b from-slate-50 to-white p-6 rounded-3xl border border-slate-200 text-center shadow-sm hover:shadow-md transition-shadow">
                                    <div class="flex justify-center -space-x-3 mb-4">
                                        @for ($i = 0; $i < 4; $i++)
                                            {{-- Usamos un delay un poco diferente para que no coincida exactamente con el oro --}}
                                            <span class="text-4xl drop-shadow-md animate-bounce"
                                                style="animation-delay: {{ $i * 0.1 + 0.5 }}s">🥈</span>
                                        @endfor
                                    </div>
                                    <p class="font-black text-slate-700 uppercase text-xs tracking-widest">2do Puesto
                                    </p>
                                    <p class="text-[10px] text-slate-500/70 font-bold mt-1">4 Medallas de Plata<br>(Una
                                        para cada integrante)</p>
                                </div>

                                {{-- BRONCE --}}
                                <div
                                    class="bg-gradient-to-b from-orange-50 to-white p-6 rounded-3xl border border-orange-200 text-center shadow-sm hover:shadow-md transition-shadow">
                                    <div class="flex justify-center -space-x-3 mb-4">
                                        @for ($i = 0; $i < 4; $i++)
                                            <span class="text-4xl drop-shadow-md animate-bounce"
                                                style="animation-delay: {{ $i * 0.1 + 1 }}s">🥉</span>
                                        @endfor
                                    </div>
                                    <p class="font-black text-orange-800 uppercase text-xs tracking-widest">3er Puesto
                                    </p>
                                    <p class="text-[10px] text-orange-700/70 font-bold mt-1">4 Medallas de
                                        Bronce<br>(Una para cada integrante)</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div x-show="tab === 'puntos'" x-transition.opacity>
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                            <div>
                                <h3 class="text-2xl font-black text-slate-800 uppercase italic tracking-tight italic">
                                    Puntos Globales (Por Equipo)</h3>
                                <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Estos puntos
                                    definen la tabla general</p>
                            </div>
                            <div class="px-4 py-2 bg-[#c5a059]/10 rounded-full border border-[#c5a059]/20">
                                <span class="text-[10px] font-black text-[#c5a059] uppercase tracking-tighter">Criterio:
                                    Resultado por Ronda</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {{-- Ganado --}}
                            <div
                                class="relative group bg-slate-900 p-8 rounded-3xl text-center shadow-2xl transition-all hover:-translate-y-2">
                                <div
                                    class="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#c5a059] text-white text-[9px] font-black px-4 py-1 rounded-full uppercase tracking-widest">
                                    Ganador</div>
                                <span class="block text-5xl font-black text-[#c5a059] mb-2">2</span>
                                <span
                                    class="text-[10px] uppercase tracking-[0.2em] font-bold text-white opacity-60">Puntos
                                    Globales</span>
                                <p class="mt-4 text-xs text-slate-400 font-medium leading-tight">Si el equipo suma más
                                    victorias individuales en la ronda.</p>
                            </div>

                            {{-- Empate --}}
                            <div
                                class="relative group bg-white border-2 border-slate-100 p-8 rounded-3xl text-center transition-all hover:-translate-y-2">
                                <span class="block text-5xl font-black text-slate-800 mb-2">1</span>
                                <span class="text-[10px] uppercase tracking-[0.2em] font-bold text-slate-400">Punto
                                    Global</span>
                                <p class="mt-4 text-xs text-slate-500 font-medium leading-tight">Si ambos equipos
                                    terminan con la misma cantidad de victorias.</p>
                            </div>

                            {{-- Perdido --}}
                            <div
                                class="relative group bg-white border-2 border-slate-100 p-8 rounded-3xl text-center transition-all hover:-translate-y-2 opacity-60">
                                <span class="block text-5xl font-black text-slate-300 mb-2">0</span>
                                <span class="text-[10px] uppercase tracking-[0.2em] font-bold text-slate-400">Puntos
                                    Globales</span>
                                <p class="mt-4 text-xs text-slate-500 font-medium leading-tight">Si el equipo tiene
                                    menos victorias que el rival en la ronda.</p>
                            </div>
                        </div>

                        {{-- Nota Aclaratoria --}}
                        <div
                            class="mt-10 p-4 bg-slate-50 rounded-2xl border-l-4 border-slate-300 flex items-center justify-between">
                            <p class="text-[11px] text-slate-500 font-bold uppercase tracking-wide">
                                <span class="text-slate-800">Nota:</span> Los puntos de las partidas individuales se
                                guardan para el desempate final.
                            </p>
                        ...
                        <div class="mt-10 space-y-8">
                            ...
                            <div class="flex items-center gap-3">
                                <h4 class="text-sm font-black text-slate-400 uppercase tracking-[0.2em]">Ejemplos
                                    Prácticos</h4>
                                <div class="flex-1 h-px bg-slate-100"></div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                                    <div class="bg-slate-900 p-4 text-center">
                                        <p class="text-[10px] font-black text-[#c5a059] uppercase tracking-widest">
                                            Escenario de Victoria</p>
                                    </div>
                                    <div class="p-6 space-y-3">
                                        <div
                                            class="flex justify-between text-[10px] font-black text-slate-400 uppercase px-2">
                                        ...
                                        <div class="space-y-2">
                                            <div
                                                ...
                                                <span class="font-bold text-slate-700">Mesa A <span
                                                        class="text-[9px] text-slate-400 mx-2">VS</span> Mesa A</span>
                                                <span class="font-black text-green-600">Gana Equipo X (+1)</span>
                                            </div>
                                            <div
                                                class="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                <span class="font-bold text-slate-700">Mesa B <span
                                                        class="text-[9px] text-slate-400 mx-2">VS</span> Mesa B</span>
                                                <span class="font-black text-blue-600">Gana Equipo Y (+1)</span>
                                            </div>
                                            <div
                                                class="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                <span class="font-bold text-slate-700">Mesa C <span
                                                        class="text-[9px] text-slate-400 mx-2">VS</span> Mesa C</span>
                                                <span class="font-black text-blue-600">Gana Equipo Y (+1)</span>
                                            </div>
                                            <div
                                                class="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                <span class="font-bold text-slate-700">Mesa D <span
                                                        class="text-[9px] text-slate-400 mx-2">VS</span> Mesa D</span>
                                                <span class="font-black text-slate-500 italic text-[11px]">Tablas (0.5
                                                    cada uno)</span>
                                            </div>
                                        </div>
                                        <div
                                            class="pt-4 border-t border-dashed border-slate-200 flex justify-between items-end">
                                            <div>
                                                <p class="text-[9px] font-black text-slate-400 uppercase">Total Puntos
                                                    Mesa</p>
                                                <p class="font-bold text-slate-600">X: 1.5 | <span
                                                        class="text-blue-600">Y: 2.5</span></p>
                                            </div>
                                            <div class="text-right">
                                                <p class="text-[9px] font-black text-[#c5a059] uppercase italic">
                                                    Resultado Final</p>
                                                <p class="font-black text-slate-900 text-lg">Equipo Y suma <span
                                                        class="text-[#c5a059]">2 pts</span> Globales</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                                    ...
                                    <div class="p-6 space-y-3">
                                        ...
                                        <div
                                            ...
                                            <div class="text-right">
                                                <p class="text-[9px] font-black text-slate-400 uppercase italic">
                                                    Resultado Final</p>
                                                <p class="font-black text-slate-900 text-lg">Ambos suman <span
                                                        class="text-slate-500">1 pt</span> Global</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="p-4 bg-blue-50 rounded-2xl border border-blue-100 flex items-center gap-3">
                                ...
                                <p class="text-xs text-blue-700 font-medium">
                                    <strong>Recuerda:</strong> Los Puntos de Mesa (X: 1.5, Y: 2.5, etc.) son los que te
                                    salvarán en caso de empate en la tabla general al final del torneo. ¡Cada media
                                    partida cuenta!
                                </p>
                            </div>
                        </div>
        ...
        <section class="mb-16" x-data="{ open: false }">
            <div @click="open = !open"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="bg-slate-800 p-1 rounded-full text-white transition-transform duration-300"
                                ...
                                <ul class="space-y-3">
                                    @foreach ($equipo->jugadores->sortBy('tablero') as $jugador)
                                        <li
                                            ...
                                            <div class="flex items-center gap-3">
                                                <span
                                                    class="{{ $colorClase['text'] == 'text-white' ? 'text-slate-400' : $colorClase['text'] }} text-lg">♟</span>
                                                <span
                                                    class="ml-1 text-[9px] font-black bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">{{ $jugador->tablero }}</span>
                                                <span
                                                    class="font-bold {{ $colorClase['text'] == 'text-white' ? 'text-slate-700' : 'text-slate-600' }}">{{ $jugador->nombre }}</span>
